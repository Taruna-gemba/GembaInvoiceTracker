...
const HoldTable = ({ invoices = [] }) => {
  const [sortConfig, setSortConfig] = useState({ key: null, direction: "asc" });

  // Ensure invoices is always an array before filtering
  const holdingInvoices = useMemo(() => {
    if (!Array.isArray(invoices)) return [];  // Prevent errors
    return invoices.filter((invoice) => invoice.status === "Holding");
  }, [invoices]);

  // Sorting logic
  const sortedData = useMemo(() => {
    if (!Array.isArray(holdingInvoices) || holdingInvoices.length === 0) return [];

    if (!sortConfig.key) return holdingInvoices;

    return [...holdingInvoices].sort((a, b) => {
      if (a[sortConfig.key] < b[sortConfig.key]) return sortConfig.direction === "asc" ? -1 : 1;
      if (a[sortConfig.key] > b[sortConfig.key]) return sortConfig.direction === "asc" ? 1 : -1;
      return 0;
    });
  }, [holdingInvoices, sortConfig]);
            ...
            {sortedData.length > 0 ? (
              sortedData.map((item, index) => (
                <tr key={index} className="border-b hover:bg-gray-50 text-sm text-gray-700">
                  ...
                  <td className="p-3">
                    <span className="px-3 py-1 rounded text-white bg-orange-500">
                      {item.status ?? "N/A"}
                    </span>
                  </td>
