...
const RejectedTable = ({ invoices = [] }) => {
  const [sortConfig, setSortConfig] = useState({ key: null, direction: "asc" });
    ...
    const sortedData = useMemo(() => {
      if (!Array.isArray(rejectedInvoices) || rejectedInvoices.length === 0) return [];
  
      if (!sortConfig.key) return rejectedInvoices;
  
      return [...rejectedInvoices].sort((a, b) => {
        if (a[sortConfig.key] < b[sortConfig.key]) return sortConfig.direction === "asc" ? -1 : 1;
        if (a[sortConfig.key] > b[sortConfig.key]) return sortConfig.direction === "asc" ? 1 : -1;
        return 0;
      });
    }, [rejectedInvoices, sortConfig]);
              ...
              {sortedData.length > 0 ? (
                sortedData.map((item, index) => (
                  <tr key={index} className="border-b hover:bg-gray-50 text-sm text-gray-700">
                    ...
                    <td className="p-3">
                      <span className="px-3 py-1 rounded text-white bg-red-500">
                        {item.status ?? "N/A"}
                      </span>
                    </td>
